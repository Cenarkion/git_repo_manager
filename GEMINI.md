...
This project, `git_repo_manager`, contains a Python script (`git_repo_manager`) designed for managing GitHub repositories directly from the command line. It interacts with the GitHub API to perform various repository management tasks.
...
2.  From a configuration file located at `~/.git_repo_manager/config.ini`. The file should have a `[github]` section with a `token` key:
    ```ini
    [github]
    token = your_github_personal_access_token
    ```
    **Note:** The directory `~/.git_repo_manager` and the file `config.ini` will be created if they don't exist. The script also attempts to set appropriate permissions for the config file (read-only for the owner).
...
*   **GitHub Token Management:** Prioritize using environment variables (`GITHUB_TOKEN`) or a dedicated configuration file (`~/.git_repo_manager/config.ini`) for storing your GitHub Personal Access Token securely.
